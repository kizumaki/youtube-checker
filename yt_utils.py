...
def to_pure_id(raw_val):
    ...
    if m_url:
        val = m_url.group(1)
        if val.lower() in ['watch', 'shorts', 'feed', 'embed', 'results']:
            return None
        s = val
    s = re.sub(r'[\s]+', '', s)
...
def get_channel_link(pure_id):
    """Trả về URL thuần cho liên kết YouTube"""
    if not pure_id:
        return ""
...
def extract_search_query(raw_url):
    ...
    if m:
        raw_q = m.group(1)
        return urllib.parse.unquote(raw_q).replace('+', ' ').strip()
    return None
...
def generate_candidate_handles_from_query(q):
    ...
    if c2_clean and c2_clean not in candidates and len(c2_clean) >= 3:
        candidates.append(c2_clean)
...
def test_all_api_keys():
    ...
    for k in keys:
        try:
            ...
            if k in exhausted_set:
                exhausted_set.remove(k)
        except Exception:
            usage[k] = 10000
...
def extract_contacts_and_socials(text_corpus):
    ...
    if ig:
        contacts['Instagram'] = f"https://instagram.com/{ig[0].rstrip('./-_,')}"
    tt = re.findall(r'(?:https?://)?(?:www\.)?tiktok\.com/@?([a-zA-Z0-9_.-]+)', corpus, re.IGNORECASE)
    if tt:
        contacts['TikTok'] = f"https://tiktok.com/@{tt[0].rstrip('./-_,')}"
    x = re.findall(r'(?:https?://)?(?:www\.)?(?:twitter\.com|x\.com)/([a-zA-Z0-9_.-]+)', corpus, re.IGNORECASE)
    if x:
        contacts['Twitter'] = f"https://x.com/{x[0].rstrip('./-_,')}"
    dc = re.findall(r'(?:https?://)?(?:www\.)?discord\.(?:gg|com/invite)/([a-zA-Z0-9_-]+)', corpus, re.IGNORECASE)
    if dc:
        contacts['Discord'] = f"https://discord.gg/{dc[0]}"
    fb = re.findall(r'(?:https?://)?(?:www\.)?facebook\.com/([a-zA-Z0-9_.-]+)', corpus, re.IGNORECASE)
    if fb:
        contacts['Facebook'] = f"https://facebook.com/{fb[0].rstrip('./-_,')}"
    return contacts

def render_social_badges_html(contacts_dict):
    if not contacts_dict:
        return "<span style='font-size:0.75rem; color:#9CA3AF;'>Chưa có thông tin MXH</span>"
    html = "<div style='margin-top: 6px;'>"
    if 'Email' in contacts_dict:
        html += f"<a href='mailto:{contacts_dict['Email']}' class='social-badge social-email'>✉️ {contacts_dict['Email']}</a>"
    if 'Instagram' in contacts_dict:
        html += f"<a href='{contacts_dict['Instagram']}' target='_blank' class='social-badge social-ig'>📸 IG</a>"
    if 'TikTok' in contacts_dict:
        html += f"<a href='{contacts_dict['TikTok']}' target='_blank' class='social-badge social-tt'>🎵 TikTok</a>"
    if 'Twitter' in contacts_dict:
        html += f"<a href='{contacts_dict['Twitter']}' target='_blank' class='social-badge social-x'>🐦 X</a>"
    if 'Discord' in contacts_dict:
        html += f"<a href='{contacts_dict['Discord']}' target='_blank' class='social-badge social-discord'>💬 Discord</a>"
    if 'Facebook' in contacts_dict:
        html += f"<a href='{contacts_dict['Facebook']}' target='_blank' class='social-badge social-fb'>📘 FB</a>"
    if 'Website' in contacts_dict:
        html += f"<a href='{contacts_dict['Website']}' target='_blank' class='social-badge social-web'>🌐 Web</a>"
    html += "</div>"
    return html
    ...
    if search_queries:
        try:
            with st.spinner(f"🔍 Đang truy vấn YouTube Search API cho {len(search_queries)} tên YouTuber..."):
                for h in get_handles_from_search_queries(list(search_queries)):
                    handles.add(h)
        except Exception:
            pass
    ...
    if video_ids:
        try:
            with st.spinner(f"🔍 Đang giải mã {len(video_ids)} Link Video sang Handle..."):
                for h in get_handles_from_video_ids(list(video_ids)):
                    handles.add(h)
        except Exception:
            pass
...
def is_within_last_90_days(date_str):
    ...
    if m_dmy:
        try:
            return 0 <= (today - datetime.date(int(m_dmy.group(3)), int(m_dmy.group(2)), int(m_dmy.group(1)))).days <= 90
        except Exception:
            pass
    m_ymd = re.match(r'^(\d{4})[-/](\d{1,2})[-/](\d{1,2})', s)
    if m_ymd:
        try:
            return 0 <= (today - datetime.date(int(m_ymd.group(1)), int(m_ymd.group(2)), int(m_ymd.group(3)))).days <= 90
        except Exception:
            pass
    try:
        return 0 <= (today - pd.to_datetime(s, dayfirst=True).date()).days <= 90
    except Exception:
        return False
...
def passes_layer1_metadata_filter(title, desc, country_code):
    ...
    if NON_LATIN_REGEX.search(text):
        return False, "Ngôn ngữ không phù hợp"
    if VIETNAMESE_UNIQUE_REGEX.search(text):
        return False, "Kênh Ngôn Ngữ Tiếng Việt"
    for kw in EXCLUDED_KEYWORDS:
        if re.search(r'\b' + re.escape(kw) + r'\b', text):
            return False, f"Loại nội dung cấm ({kw.upper()})"
    return True, "OK"
...
def process_tab1_single_handle(p_id, db_existing_map, api_keys, exhausted_keys=None):
    ...
    logs = list(l1)
    if not cid:
        return "REJECTED", {"Handle": f"@{p_id}" if not p_id.startswith('UC') else p_id, "Tên Kênh": p_id.upper(), "Trạng thái": "❌ Không tìm thấy kênh", "Lý do loại": "Không tồn tại trên YT"}, logs
...
def process_single_candidate(item, min_subs_choice, min_duration_choice, db_existing_set, api_keys, exhausted_keys=None):
    ...
    base_data = {
        "Handle": f"@{c_handle}" if not c_handle.startswith('UC') else c_handle, 
        "Link Kênh": c_url, 
        "Tên Kênh": c_title, 
